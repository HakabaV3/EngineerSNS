var util = require('../util.js'),
    ...
    QueryTextPart = require('./querytextpart.js'),
    QueryBindingPart = require('./querybindingpart.js'),
    QueryViewPart = require('./queryviewpart.js'),
    ...
    queryParts = queryPartDatas.map(function(queryPartData) {
        switch (queryPartData.type) {
            case QueryPartType.TEXT:
                return new QueryTextPart(queryPartData.data);
                break;
            case QueryPartType.BINDING:
                return new QueryBindingPart(queryPartData.data.copy(bindTarget));
                break;
            case QueryPartType.VIEW:
                v = new queryPartData.data.viewConstructor();
                if (util.isObject(childViews)) childViews[queryPartData.data.name] = v;
                return new QueryViewPart(v);
                break;
        }
    });
    return new Query(node, queryData.attrName, queryParts);
};
/**
 *  ノードパスからノードを特定する
 */
...
Template.parseTemplateNodeRecursive = function(rootNode, result, nodePath) {
    ...
    util.forEach(rootNode.childNodes, function(child, i) {
    ...
    while (ma = regTag.exec(queryText)) {
        ...
        switch (tagTextParts[0].toUpperCase()) {
            ...
            default:
                //binding
                datas.push({
                    type: QueryPart.Type.BINDING,
                    data: new Binding(null, tagText.split('.'))
                });
                break;
        }
    };
...
Template.parseQueryPartsForView = function(tagTextParts) {
    ...
    if (result['class'] && util.isFunction(global[result['class']])) {
